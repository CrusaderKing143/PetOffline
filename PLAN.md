...
本计划等待用户确认。Approval Gate 通过前只允许资料/计划修订，不开始或继续大规模实现。

当前 Git 已有 `d08c995 feat: establish Pet Offline Unity foundation`，工作树还存在未提交的 Milestone 1 代码改动。它们都超出了本轮“只做资料归一与计划”的范围，因此本轮全部冻结：不继续扩展、不回退、不提交，也不把它们视作计划获批。计划确认后的第一项工作，是逐文件对照本计划审计这些生成物，再按用户指示保留、修订或另行处理。

- 完整两关可玩性、生产 UGUI、全部 P0 测试、双结局 Smoke、截图和 Windows Build 均未验收。
- 工作树中已有证据文件记录 EditMode 2/2、PlayMode 11/11 与 Validation PASS；它们只证明当前 Foundation/Day 1 灰盒范围，不代表 Day 2、生产 UI、Standalone 或 Definition of Done。
...
纸面闭环审计结论：计划已经覆盖 `Title → Day 1 → Day 1 Report/Ending → Day 2 → Day 2 Report → Choice → Restore/Keep Quiet → Return/Restart` 的每个转移；两个结局使用独立空白存档运行，另有跨进程 Continue 运行。该结论只表示计划无缺口，不表示当前 Unity 工程已经完成或通过验收。
...
命令状态分两类：Setup、Validation、EditMode、PlayMode 当前已有入口，Setup/Validation 已使用显式 `EditorApplication.Exit(0/1)`。`WindowsBuild` 与 `StandaloneSmokeDriver` 是本计划要求在 Milestone 5 前实现的最终入口，当前不存在，未实现前不得声称对应命令已运行成功。Unity 6.3 Test Runner 在本机写完 XML 后可能保留 Editor 进程，因此测试命令不能依赖 `-quit`；包装脚本只终止自己启动的精确 PID，不能关闭其他 Unity 项目。
...
`WindowsBuild` 必须显式使用 `BuildTarget.StandaloneWindows64` 和 x86_64，并检查 `BuildReport.summary.result == BuildResult.Succeeded`。本机当前只安装了 Windows x64 Mono Player variation，没有 IL2CPP variation，因此本垂直切片默认构建 Mono x64；BuildReport 记录该事实，计划不擅自安装 IL2CPP。

在自定义 `WindowsBuild` 尚未实现时，只允许用以下 Unity 原生 fallback 验证平台模块能否产出 Player；它不能替代最终 BuildReport 门槛：

    & $Unity -batchmode -nographics -quit -projectPath $Project -buildTarget StandaloneWindows64 -development -buildWindows64Player "$Builds\Development\PetOffline.exe" -logFile "$Results\BuildDevelopment.log"
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path "$Builds\Development\PetOffline.exe")) { throw 'Development build failed' }

    & $Unity -batchmode -nographics -quit -projectPath $Project -buildTarget StandaloneWindows64 -buildWindows64Player "$Builds\PetOffline.exe" -logFile "$Results\BuildRelease.log"
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path "$Builds\PetOffline.exe")) { throw 'Release build failed' }
...
180 秒超时只适用于 Development Smoke 的缩时测试配置：它只缩短对话等待、Goal Hold、Boss Call 和 Sun/Confirm 秒数，仍必须经过真实 Bootstrap/additive World Scene、Input、Physics2D、世界触发、状态机、报告、选择和结局。正常 Release 配置仍按 12–15 分钟人工验收。
...
Approval Gate：未通过，等待用户确认。本文件输出后暂停。确认前不继续现有 Milestone 1 改动；也不删除或回退用户的 AGENTS.md、Main/SampleScene、Reference 资料或其他所有权不明内容。
...
| 已有 Foundation commit 与未提交 Milestone 1 改动超出本轮范围 | 全部冻结；获批后先审计，不把最小测试当作完成 | 用户决定保留、修订或另行处理；本计划不擅自回退 |
